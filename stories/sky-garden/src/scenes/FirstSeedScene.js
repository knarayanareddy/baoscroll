...
export class FirstSeedScene extends BaseScene {
  ...
  camera(local, t, cam) {
    ...
    if (local < 0.25) {
      ...
      cam.position.set(-4.4 + plantT * 0.5, 0.7, 2.4);
      cam.lookAt(-3.9, -0.3, 0.1);
    } else if (local < 0.55) {
      // wide side view: the bridge is the subject
      cam.position.set(lerp(-2, 0, growT), 1.3, 7.6 - growT * 1.2);
      cam.lookAt(0, -0.4, 0);
    } else if (local < 0.82) {
      // track the crossing at foot level
      const u = 0.02 + crossT * 0.96;
      const p = this.vineCurve.getPoint(u);
      cam.position.set(p.x + 1.1, p.y + 1.1, 3.6);
      cam.lookAt(p.x, p.y + 0.2, 0);
    } else {
      // arrival on the right island, wind from beyond
      cam.position.set(5.2 + arriveT * 0.8, 1.5, 4.6 - arriveT);
