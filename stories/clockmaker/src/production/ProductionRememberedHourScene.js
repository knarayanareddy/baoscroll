...
export class ProductionRememberedHourScene extends ProductionBaseScene {
  build() {
    ...
    this.unspentHour = new UnspentHour(kit,{particleCount:this.tiered(58,36,18),scale:.42}).attachTo(this.watch.root,new THREE.Vector3(0,0,.18));
    ...
    const mentorArmL = new THREE.Mesh(new THREE.PlaneGeometry(.16, .78), new THREE.MeshBasicMaterial({ color:'#ffe5aa', transparent:true, opacity:0, side:THREE.DoubleSide, depthWrite:false, blending:THREE.AdditiveBlending })); mentorArmL.position.set(-.42,1.32,.02); mentorArmL.rotation.z=.55; this.mentor.add(mentorArmL);
    const mentorArmR = new THREE.Mesh(new THREE.PlaneGeometry(.16, .78), new THREE.MeshBasicMaterial({ color:'#ffe5aa', transparent:true, opacity:0, side:THREE.DoubleSide, depthWrite:false, blending:THREE.AdditiveBlending })); mentorArmR.position.set(.42,1.32,.02); mentorArmR.rotation.z=-.55; this.mentor.add(mentorArmR);
    this.mentor.userData = { coat: mentorCoat, head: mentorHead, glow: mentorGlow, armL:mentorArmL, armR:mentorArmR };
    ...
    this.timeField=experience.effects.timeFractureField({radius:3.2});this.timeField.position.set(0,1.8,-.9);g.add(this.timeField);
    // Mentor work lines are material memory rather than objects: they trace
    // a repair gesture across the bench as the remembered room coheres.
    this.memoryLines=[];for(let i=0;i<18;i++){const line=new THREE.Mesh(new THREE.PlaneGeometry(.025,.5+(i%4)*.15),new THREE.MeshBasicMaterial({color:i%3?'#ffd47b':'#8fb7d1',transparent:true,opacity:0,depthWrite:false,blending:THREE.AdditiveBlending}));line.userData={base:new THREE.Vector3(-1.8+(i%9)*.42,1.3,-.35+Math.floor(i/9)*.75),phase:i*.39};line.position.copy(line.userData.base);line.rotation.z=(i%5-.5)*.18;g.add(line);this.memoryLines.push(line);}
    this.thread = experience.effects.redThreadGlow([new THREE.Vector3(-.14, 1.3, .44), new THREE.Vector3(.78, 1.55, .2), new THREE.Vector3(2.42, 1.48, -.75)]); this.thread.visible = false; g.add(this.thread);
  ...
  update(p, time) {
    ...
    this.mentor.userData.armL.material.opacity = mentorOpacity * .5;
    this.mentor.userData.armR.material.opacity = mentorOpacity * .5;
    this.mentor.userData.armL.rotation.z = .55 + Math.sin(time * .8) * .22 * memory;
    this.mentor.userData.armR.rotation.z = -.55 - Math.sin(time * .8) * .22 * memory;
    this.mentor.userData.glow.intensity = mentorOpacity * 2.8;
    this.mentor.position.y = .16 + Math.sin(time * .62) * .055;
    this.memoryLines.forEach(line=>{const appear=memory*(1-collapse);line.position.y=line.userData.base.y+Math.sin(time*1.2+line.userData.phase)*.09*appear;line.rotation.z=(line.userData.phase%1-.5)*.25+Math.sin(time*.8+line.userData.phase)*.18*appear;line.material.opacity=appear*(.18+.3*Math.sin(time+line.userData.phase));});
    this.timeField.userData.setTimeFracture(materialProgress+collapse*.7,time);this.thread.visible = resist > .14;
    this.thread.userData.setThread(resist * (1 - collapse));
    ...
    this.timeLight.position.copy(this.watch.root.position);this.unspentHour.setMood(collapse>.05?'hostile':'remembering');this.unspentHour.setProgress(collapse>.05?collapse:materialProgress);this.unspentHour.update(time,0,1);
    ...
    this.debugState = { contacts:this.apprentice.captureContacts(), progress: p, reconstruction: materialProgress, mentorOpacity, refusal: collapse, redThreadVisible: this.thread.visible, finite: this.fragments.every((node) => Number.isFinite(node.position.x) && Number.isFinite(node.position.y) && Number.isFinite(node.position.z)) };
  }
}
