...
export class ClockmakerStoryExperience {
  constructor(canvas){this.canvas=canvas;this.scene=new THREE.Scene();this.world=new THREE.Group();this.scene.add(this.world);this.renderer=new THREE.WebGLRenderer({canvas,antialias:true});this.renderer.outputColorSpace=THREE.SRGBColorSpace;this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,2));this.renderer.setSize(innerWidth,innerHeight);this.camera=new CameraRig();this._pos=new THREE.Vector3();this._look=new THREE.Vector3();this.kit=new ClockworkPaperKit();this.lights={hemi:new THREE.HemisphereLight('#ead8ac','#1b2a42',.5),key:new THREE.DirectionalLight('#ffd47b',.55),rim:new THREE.DirectionalLight('#7ca1c7',.5),accent:new THREE.PointLight('#ffd47b',0,18,2)};this.lights.key.position.set(4,6,4);this.lights.rim.position.set(-4,3,-5);this.scene.add(...Object.values(this.lights));this.scenes=[new HoldingScene(this,0),new HoldingScene(this,1),new ProductionGearTowerScene(this,2),new ProductionRememberedHourScene(this,3),new HoldingScene(this,4),new HoldingScene(this,5)];this.scenes.forEach(s=>s.ensure());this.elapsed=0;window.addEventListener('resize',()=>this.resize());}
